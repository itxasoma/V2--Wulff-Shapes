...
def save_csv(results, metals, surfaces, outfile: Path):
    """Save to outfile — always an absolute path resolved from CWD."""
    outfile = Path.cwd() / outfile.name   # force save in CWD, not in ROOT
...
def main():
    ...
    csv_path = Path(args.csv)          # just the filename; CWD is used in save_csv
